...
async def check_and_update_roles(member: discord.Member):
    ...
    for rank in reversed(RANK_ROLES):
            ...
            for rank_info in reversed(RANK_ROLES):
                for role in member.roles:
                    if role.name == rank_info["name"]:
                        member_rank_weight = rank_info["weight"];
                        break
...
async def resolve_world_boss(channel: discord.TextChannel):
    ...
    if boss_hp <= 0:
        embed.title = f"🎉 WORLD BOSS DEFEATED! 🎉"
        embed.description = f"**{boss_title}** has been vanquished by the brave soldiers of Davis!"
        ...
        embed.title = f"☠️ WORLD BOSS SURVIVED ☠️"
        embed.description = f"**{boss_title}** was too powerful and escaped before it could be defeated."
...
# --- Game Feature Toggle Commands ---
@bot.command()
@commands.is_owner()
async def gameon(ctx):
    """(Owner only) Enables all game-related features."""
    global game_features_enabled
    if game_features_enabled:
        return await ctx.reply("Game features are already enabled.")
    game_features_enabled = True
    if not attack_scheduler.is_running():
        attack_scheduler.start()
...
async def gameoff(ctx):
    """(Owner only) Disables all game-related features."""
    global game_features_enabled
    if not game_features_enabled:
        return await ctx.reply("Game features are already disabled.")
    game_features_enabled = False
    if attack_scheduler.is_running():
        attack_scheduler.cancel()
    await ctx.reply("❌ Game features have been **DISABLED**. The attack scheduler is now stopped.")


# --- Utility Commands (Always Active) ---
@bot.command(name="help")
async def help_command(ctx):
    """Shows a list of available commands for users."""
    embed = discord.Embed(
        title="Davis Defense Bot Commands",
...
async def announce(ctx, *, message: str):
    ...
    if not channel:
        return await ctx.reply(f"I couldn't find the `#{ANNOUNCEMENT_CHANNEL_NAME}` channel.")
...
async def ping(ctx):
    ping_message = (
        "> PING: HENRY DAVIS DEFENSE ROBOT [ID: HD-DRX-0923]\n"
        "> SIGNAL STRENGTH: MAX\n"
...
async def worldboss(ctx, *, params: str):
    ...
    if boss_event_active:
        return await ctx.reply("A world boss event is already in progress.")
    try:
...
async def hit(ctx):
    ...
    if ctx.author.id in boss_participants:
        return await ctx.reply("You have already attacked this boss!", delete_after=10)
    ...
    boss_hp -= damage
    await ctx.send(
        f"**{ctx.author.display_name}** hits **{boss_title}** for **{damage}** damage! (HP: {max(0, boss_hp)}/{boss_max_hp})")
